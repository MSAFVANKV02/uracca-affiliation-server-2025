...
export const getAffiliatePerformanceTable = async (req, res, next) => {
  try {
    ...
    const actionMap = new Map(actions.map((a) => [a._id.toString(), a]));
...
export const analysisTablesOfUserInAdmin = async (req, res, next) => {
  try {
    const adminId = req.admin._id;
    const userId = req.params.userId;

    if (!adminId || !userId) {
      throw new MissingFieldError("admin and user ids are required");
    }

    /* ------------------------------------------------------------------
     1️⃣ CAMPAIGNS TABLE (Active Campaigns)
    ------------------------------------------------------------------ */

    const campaigns = await Campaign.find({
      userId: userId,
      "company.accountId": adminId,
    });
    if (!campaigns) {
      throw new NotFoundError("Campaign Not Found");
    }
    const campaignTable = campaigns.map((c) => ({
      campaignName: c.product?.title ?? "N/A",
      startDate: c.createdAt,
      sale: c.commissionDetails?.totalCommission || 0,
      status: c.status,
      clicks: c.clicks || 0,
      conversions: c.ordersCount || 0,
      earnings: c.commissionDetails?.totalCommissionWithTds || 0,
    }));

    /* ------------------------------------------------------------------
     2️⃣ COMMISSIONS (Send ALL like others)
    ------------------------------------------------------------------ */
    ...
    /* ------------------------------------------------------------------
     3️⃣ TDS HISTORY (Derived from COMMISSIONS)
    ------------------------------------------------------------------ */
    const tdsHistory = commissions
      .filter((c) => c.tdsAmount > 0)
      .map((c) => ({
        orderId: c.orderId,
        date: c.createdAt,
        ...
        tds: {
          ...
          percentage:
            c.commissionAmount > 0
              ? Math.round((c.tdsAmount / c.commissionAmount) * 100)
              : 0,
        },
      }));
    ...
    // const withdrawalTable = withdrawals.map((w) => ({
    //   transactionId: w._id,
    //   date: w.createdAt,
    //   amount: w.withdrawalAmount,
    //   status: w.status,
    //   method:
    //     w.paymentMethod === "ONLINE"
    //       ? w.onlineMethod?.method
    //       : w.paymentMethod,
    //   account:
    //     w.onlineMethod?.upiId ||
    //     w.onlineMethod?.bank?.accountNumber ||
    //     "",
    // }));
